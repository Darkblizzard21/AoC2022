...
namespace AoC2022.util
{
    public static class ArrayExtensions
    {
        ...
        public static T[][][] New3DWithDefault<T>(int xSize, int ySize, int zSize, Func<T> defaultGenerator) => EnumerableGeneration.Sequence(xSize, () => EnumerableGeneration.Sequence2D(ySize, zSize, defaultGenerator).ToArray2D()).ToArray();
        public static IEnumerable<IntVector3> Vectors<T>(this T[][][] values) => EnumerableGeneration.FromTo(0, values.Length)
            .SelectMany(x => EnumerableGeneration.FromTo(0, values[x].Length)
            .SelectMany(y => EnumerableGeneration.FromTo(0, values[x][y].Length).Select(z => new IntVector3(x, y, z))));
